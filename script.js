...
copyDataBtn.addEventListener('click', () => {
    // Generate the JS code to replace the initialData array
    const dataStr = JSON.stringify(players, null, 4);
    const replacementCode = `const initialData = ${dataStr};`;
    
    // Copy to clipboard
    navigator.clipboard.writeText(replacementCode).then(() => {
        alert("✅ Data successfully copied to clipboard!\n\nBecause this is a local website without a backend server, your browser cannot directly edit the files on your disk.\n\nTo make changes permanent for everyone:\n1. Open 'script.js' in a text editor (like Notepad or VS Code)\n2. Select the entire 'const initialData = [ ... ];' section at the top\n3. Paste to replace it with the copied text\n4. Save the file!");
    }).catch(err => {
        alert("Clipboard copy failed. The data has been logged to the Developer Console (F12) instead.");
